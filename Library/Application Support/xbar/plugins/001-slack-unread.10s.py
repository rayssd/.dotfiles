...
users_counts_url = 'https://slack.com/api/users.counts'
...
def get_JSON_output(url, token):
    if debug_mode:
        print("Processing API key: %s" % token)
    r = requests.get(url + '?token=' + token, headers)
    return r


def get_unreads(url, token):
    unread = 0

    # insert conversation_info_url to retrieve last_read here
    try:
        r = get_JSON_output(url, token)
        print(r.text)
        channels = json.loads(r.text)['channels']
        ims = json.loads(r.text)['ims']
        groups = json.loads(r.text)['groups']

        for channel in channels:
            unread += channel['mention_count_display']
        for im in ims:
            unread += im['dm_count']
        for group in groups:
            unread += group['mention_count_display']
...
if __name__ == '__main__':
    unread_count = get_unreads(users_counts_url, token)
